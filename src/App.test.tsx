...
describe('CodeHarness workbench', () => {
  ...
  it('switches to changes and accepts a file change', async () => {
    ...
    await user.click(screen.getByRole('button', { name: '接受 src/api/login.ts' }));
  ...
  it('confirms before cancelling the active task', async () => {
    ...
    expect(screen.getByRole('dialog', { name: '取消当前任务' })).toBeInTheDocument();
    const cancelButtons = screen.getAllByRole('button', { name: '取消任务' });
    await user.click(
      cancelButtons.find((button) => !button.hasAttribute('aria-label')) ?? cancelButtons[0]
    );
  ...
  it('opens the selected search result at its matching line', async () => {
    ...
    await waitFor(() => expect(screen.getByText('src/harness/runner.ts')).toBeInTheDocument());
  ...
  it('creates a new task and sends a message', async () => {
    ...
    expect(await screen.findByText(/已把目标加入当前任务/)).toBeInTheDocument();
  });
});
