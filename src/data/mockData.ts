...
export const initialSnapshot: WorkspaceSnapshot = {
  ...
  messages: {
    ...
    'session-index': [
      ...
      },
      {
        id: 'm4',
        role: 'assistant',
        content: '索引从文件扫描开始，经过语言识别、符号提取和关系持久化，最后生成项目概览。',
        createdAt: '昨天'
      }
    ],
    'session-tests': [
  ...
  files: {
    'src/app/App.tsx': {
      path: 'src/app/App.tsx',
      language: 'tsx',
      content: `import { Workbench } from "../components/Workbench";\n\nexport function App() {\n  return <Workbench />;\n}\n`
    },
    'src/app/main.tsx': {
      path: 'src/app/main.tsx',
      language: 'tsx',
    ...
    'src/api/session.ts': {
      path: 'src/api/session.ts',
      language: 'typescript',
      content: `export async function getSession(token: string) {\n  return store.sessions.find((item) => item.token === token);\n}\n`
    },
    ...
    'src/types.ts': {
      path: 'src/types.ts',
      language: 'typescript',
      content: `export type TaskStatus = "idle" | "running" | "waiting" | "completed";\n`
    },
    'tests/login.test.ts': {
      path: 'tests/login.test.ts',
      language: 'typescript',
      content: `it("returns 401 when the session is missing", async () => {\n  const response = await login(requestWithToken("expired"));\n  expect(response.status).toBe(401);\n});\n`
    },
    'README.md': {
      path: 'README.md',
      language: 'markdown',
