...
export function DiffViewer() {
  ...
  const pending = snapshot.changes.filter((change) => change.decision === 'pending');
  const additions = snapshot.changes.reduce((total, change) => total + change.additions, 0);
  const deletions = snapshot.changes.reduce((total, change) => total + change.deletions, 0);
  const canApply =
    ...
    <div className="diff-viewer">
      ...
      <div className="diff-files">
        {snapshot.changes.map((change) => {
          ...
          return (
            <section className={`diff-file diff-file--${change.decision}`} key={change.id}>
              <header className="diff-file__header">
                ...
                <div className="diff-file__controls">
                  <span className={`diff-decision diff-decision--${change.decision}`}>
      ...
      <Modal
        open={confirmApply}
        title="应用已接受变更"
        description="已接受的文件将安全写回原始项目。"
        onClose={() => setConfirmApply(false)}
        footer={
          <>
            <button className="button button--secondary" onClick={() => setConfirmApply(false)}>
              继续审阅
            </button>
            <button
      ...
      >
        <p className="confirmation-copy">
          后端会再次校验任务基线；若原始项目已并发修改，本次应用将被拒绝。
        </p>
      </Modal>
      <Modal
        ...
        footer={
          <>
            <button className="button button--secondary" onClick={() => setConfirmRollback(false)}>
              保留变更
            </button>
            <button
