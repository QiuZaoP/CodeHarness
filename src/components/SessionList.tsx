import { Ban, CircleCheck, CircleDashed, CircleEllipsis, MessageSquarePlus } from 'lucide-react';
        ...
        {snapshot.sessions.map((session) => (
          <button
            ...
            className={`session-item ${
              session.id === snapshot.activeSessionId ? 'session-item--active' : ''
            }`}
          ...
          >
            ...
            {session.unread ? <span className="session-item__unread" aria-label="未读" /> : null}
          </button>
        ))}
      </div>
    </div>
