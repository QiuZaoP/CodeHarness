...
import type { DecisionRequest } from '../src/ports/model-gateway.ts';
import { contractSchemaVersion } from '../src/types.ts';
...
const request: DecisionRequest = {
  ...
  context: [{ reference: { ref: 'goal', kind: 'SUMMARY', source: 'user-goal' }, content: 'Return a COMPLETE decision for a smoke test.' }],
  availableTools: []
};
...
test('DeepSeek live smoke test', { skip: !enabled }, async () => {
  ...
  assert.ok(['TOOL_CALL', 'PLAN_UPDATE', 'ASK_USER', 'VERIFY', 'COMPLETE'].includes(response.decision.type));
