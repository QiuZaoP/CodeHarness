...
import type { GatewayConfig } from '../src/model-gateway/config.ts';
import { loadGatewayConfig, resolveApiKey } from '../src/model-gateway/config.ts';
import { ModelGatewayError } from '../src/model-gateway/errors.ts';
import type { FetchLike, HttpResponse } from '../src/model-gateway/http.ts';
import { GatewayMetrics } from '../src/model-gateway/metrics.ts';
import { parseModelDecision } from '../src/model-gateway/structured.ts';
import { DeepSeekModelGateway } from '../src/adapters/deepseek-model-gateway.ts';
import { FallbackModelGateway } from '../src/adapters/fallback-model-gateway.ts';
import { GuardedModelGateway } from '../src/adapters/guarded-model-gateway.ts';
import type {
  DecisionRequest,
  DecisionResponse,
  EmbeddingRequest,
  ...
  SummaryRequest,
  SummaryResponse
} from '../src/ports/model-gateway.ts';
import { contractSchemaVersion, type RunState } from '../src/types.ts';
...
function response(body: string, status = 200, headerValues: Record<string, string> = {}): HttpResponse {
  return {
    status,
...
function streamResponse(content: string, usage = { prompt_tokens: 12, completion_tokens: 8 }): HttpResponse {
  const sse = [
...
const decisionRequest: DecisionRequest = {
  runState,
  context: [{ reference: { ref: 'goal', kind: 'SUMMARY', source: 'user-goal' }, content: 'Inspect the repository' }],
  availableTools: [{
    name: 'read_file',
    version: '1.0.0',
...
test('validates every supported structured decision type and rejects invalid JSON', () => {
  assert.equal(parseModelDecision('{"type":"COMPLETE","reason":"done","summary":"ok"}').type, 'COMPLETE');
  assert.equal(parseModelDecision('{"type":"COMPLETE","reason":"done","expectedObservation":null,"summary":"ok"}').type, 'COMPLETE');
  assert.equal(parseModelDecision('{"type":"VERIFY","reason":"check","commands":["npm test"]}').type, 'VERIFY');
  assert.equal(parseModelDecision('{"type":"ASK_USER","reason":"need input","question":"Continue?"}').type, 'ASK_USER');
  assert.equal(parseModelDecision('{"type":"TOOL_CALL","reason":"inspect","tool":{"name":"read_file","arguments":{}}}').type, 'TOOL_CALL');
  assert.throws(
    () => parseModelDecision('{"type":"TOOL_CALL","reason":"inspect","tool":{"name":"unknown_tool","arguments":{}}}'),
    (error: unknown) => error instanceof ModelGatewayError &&
      ((error.details as { category?: unknown } | undefined)?.category === 'INVALID_RESPONSE')
  );
  assert.equal(parseModelDecision('{"type":"PLAN_UPDATE","reason":"plan","plan":{"goal":"g","assumptions":[],"steps":[{"id":"one","title":"Inspect","status":"PENDING"}],"verification":[]}}').type, 'PLAN_UPDATE');
  assert.throws(() => parseModelDecision('not-json'), (error: unknown) => {
...
test('calls DeepSeek-compatible chat, validates the decision, emits stream events, and records metrics', async () => {
  ...
  const result = await gateway.decide(decisionRequest, new AbortController().signal, (event) => events.push(event.type));
...
test('retries rate limits, honors retry-after, and does not leak the API key', async () => {
  ...
  const gateway = new DeepSeekModelGateway(config({ maxRetries: 1 }), { fetchImpl, sleep: async () => undefined });
  await gateway.summarize({ goal: 'g', observations: [], changedFiles: [] }, new AbortController().signal);
  ...
  await assert.rejects(
    async () => {
      ...
      await broken.summarize({ goal: 'g', observations: [], changedFiles: [] }, new AbortController().signal);
    },
    (error: unknown) => {
...
test('times out an unresponsive provider and propagates caller cancellation', async () => {
  ...
  await assert.rejects(
    () => gateway.summarize({ goal: 'g', observations: [], changedFiles: [] }, new AbortController().signal),
    (error: unknown) => error instanceof ModelGatewayError && (error.details as { category: string }).category === 'TIMEOUT'
  );
...
test('limits streamed response bytes and records malformed decisions as failures', async () => {
  ...
  await assert.rejects(
    () => tooLarge.summarize({ goal: 'g', observations: [], changedFiles: [] }, new AbortController().signal),
    (error: unknown) => error instanceof ModelGatewayError && (error.details as { category: string }).category === 'INVALID_RESPONSE'
  );
...
test('opens a circuit after repeated provider failures', async () => {
  ...
  await assert.rejects(
    ...
    (error: unknown) => error instanceof ModelGatewayError && (error.details as { category: string }).category === 'CIRCUIT_OPEN'
  );
});

test('reports that embeddings need a separately configured embedding route', async () => {
  const gateway = new DeepSeekModelGateway(config(), { fetchImpl: async () => streamResponse('unused') });
  await assert.rejects(
    () => gateway.embed({ inputs: ['hello'] }, new AbortController().signal),
    (error: unknown) => error instanceof ModelGatewayError && (error.details as { category: string }).category === 'NOT_SUPPORTED'
  );
});
...
test('uses configurable OpenAI-compatible embedding and rerank routes', async () => {
  ...
  const rerank = await gateway.rerank({ query: 'hello', documents: ['one', 'two'] }, new AbortController().signal);
...
function gatewayFixture(response: DecisionResponse): ModelGateway {
  return {
    ...
    async summarize(): Promise<SummaryResponse> { return { summary: 'fallback', usage: { inputTokens: 0, outputTokens: 0 } }; },
    async embed(_request: EmbeddingRequest): Promise<EmbeddingResponse> { return { vectors: [[1]], model: 'fallback', provider: 'fallback', usage: { inputTokens: 0 } }; }
  };
}

test('falls back to the secondary gateway for provider failures', async () => {
  const primary: ModelGateway = {
    async decide(): Promise<DecisionResponse> { throw new ModelGatewayError('primary down', 'PROVIDER'); },
    async summarize(): Promise<SummaryResponse> { throw new ModelGatewayError('primary down', 'PROVIDER'); },
    async embed(): Promise<EmbeddingResponse> { throw new ModelGatewayError('primary down', 'PROVIDER'); }
  };
  ...
  assert.equal((await gateway.decide(decisionRequest, new AbortController().signal)).provider, 'backup');
});
...
test('does not fall back after a primary stream has already emitted content', async () => {
  ...
  const primary: ModelGateway = {
    ...
    async summarize() { throw new ModelGatewayError('stream interrupted', 'PROVIDER'); },
    async embed() { throw new ModelGatewayError('primary down', 'PROVIDER'); }
  };
  const fallback: ModelGateway = {
    async decide() { fallbackCalls += 1; throw new Error('should not run'); },
    async summarize(): Promise<SummaryResponse> { return { summary: 'fallback', usage: { inputTokens: 0, outputTokens: 0 } }; },
    async embed(_request: EmbeddingRequest): Promise<EmbeddingResponse> { return { vectors: [[1]], model: 'fallback', provider: 'fallback', usage: { inputTokens: 0 } }; }
  };
  const gateway = new FallbackModelGateway(primary, fallback);
