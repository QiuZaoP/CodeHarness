import { toolNames, type ToolName } from '../contract-values.ts';
import type { ModelDecision, TaskPlan } from '../types.ts';
...
function nonEmptyString(value: unknown, field: string): string {
  if (typeof value !== 'string' || !value.trim()) {
    throw new ModelGatewayError(`Structured model output has an invalid ${field}`, 'INVALID_RESPONSE', {
      field
    }, 502);
  }
  return value.trim();
...
function toolName(value: unknown): ToolName {
  if (typeof value !== 'string' || !toolNames.includes(value as ToolName)) {
    throw new ModelGatewayError('Model tool name is not supported', 'INVALID_RESPONSE', {
      field: 'tool.name'
    }, 502);
  }
...
function parseJsonText(content: string): unknown {
  ...
  try {
    ...
    throw new ModelGatewayError('Model structured output is not valid JSON', 'INVALID_RESPONSE', {}, 502);
  }
}

function parsePlan(value: unknown): TaskPlan {
  const object = record(value);
  if (!object) throw new ModelGatewayError('Model plan is not an object', 'INVALID_RESPONSE', {}, 502);
  const stepsValue = object.steps;
  if (!Array.isArray(stepsValue) || stepsValue.length === 0) {
    throw new ModelGatewayError('Model plan must contain steps', 'INVALID_RESPONSE', { field: 'plan.steps' }, 502);
  }
  const steps = stepsValue.map((step) => {
    const item = record(step);
    if (!item) throw new ModelGatewayError('Model plan step is invalid', 'INVALID_RESPONSE', {}, 502);
    const status = item.status;
    if (status !== 'PENDING' && status !== 'RUNNING' && status !== 'DONE') {
      throw new ModelGatewayError('Model plan step status is invalid', 'INVALID_RESPONSE', {
...
export function parseModelDecision(content: string): ModelDecision {
  ...
  if (!object) throw new ModelGatewayError('Model decision must be a JSON object', 'INVALID_RESPONSE', {}, 502);
  ...
  switch (type) {
    case 'TOOL_CALL': {
      ...
      if (!tool || typeof args !== 'object' || args === null || Array.isArray(args)) {
        throw new ModelGatewayError('Model tool call is invalid', 'INVALID_RESPONSE', { field: 'tool' }, 502);
      }
      return {
        ...base,
        type,
    ...
    case 'ASK_USER':
      return { ...base, type, question: nonEmptyString(object.question, 'question') } as ModelDecision;
    case 'VERIFY':
    ...
    default:
      throw new ModelGatewayError('Model decision type is not supported', 'INVALID_RESPONSE', {
        field: 'type'
      }, 502);
  }
}
