...
import { isAbortError, isRetryableFailure } from './errors.js';
...
export async function withRetries<T>(
  ...
  while (true) {
    ...
    try {
      ...
      const details =
        error instanceof ModelGatewayError && error.details && typeof error.details === 'object'
...
export async function withTimeout<T>(
  ...
  const timeoutPromise = new Promise<never>((_, reject) => {
    timer = setTimeout(() => {
      timedOut = true;
      const timeout = new ModelGatewayError(
  ...
  try {
    ...
    if (externalSignal.aborted) throw externalSignal.reason;
    if (timedOut)
      throw new ModelGatewayError('Model request timed out', 'TIMEOUT', { timeoutMs }, 504);
    throw error;
  } finally {
