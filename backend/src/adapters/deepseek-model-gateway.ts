...
import { withTimeout, type FetchLike, type Sleep } from '../model-gateway/http.js';
import { OpenAICompatibleProvider, type ProviderUsage } from './openai-compatible-provider.js';
...
export class DeepSeekModelGateway implements ModelGateway {
  ...
  constructor(
    ...
    this.summaryCircuit = new CircuitBreaker(
      config.circuitFailureThreshold,
      config.circuitCooldownMs
    );
    this.embeddingCircuit = new CircuitBreaker(
      config.circuitFailureThreshold,
      config.circuitCooldownMs
    );
    this.rerankCircuit = new CircuitBreaker(
      config.circuitFailureThreshold,
      config.circuitCooldownMs
    );
  }
      ...
      const result = await this.run(
        ...
        async (controlledSignal) => {
          const response = await this.chatProvider.chat(buildDecisionMessages(request), {
            ...
            onUsage: (usage) =>
              onStreamEvent?.({ type: 'USAGE', usage: modelUsage(this.config.chat, usage) })
          });
  ...
  async summarize(
    ...
    try {
      const result = await this.run(
        ...
        async (controlledSignal) => {
          const response = await this.summaryProvider.chat(buildSummaryMessages(request), {
            ...
            onUsage: (usage) =>
              onStreamEvent?.({ type: 'USAGE', usage: modelUsage(this.config.summary, usage) })
          });
          retries = response.retries;
          return response;
        }
      ...
      if (!summary)
        throw new ModelGatewayError('Model summary is empty', 'INVALID_RESPONSE', {}, 502);
      const durationMs = performance.now() - started;
      this.metrics.record('summarize', { ok: true, durationMs, usage, retries });
      return { summary, usage };
    } catch (error) {
      this.metrics.record('summarize', {
        ok: false,
        durationMs: performance.now() - started,
        retries
      });
  ...
  async embed(request: EmbeddingRequest, signal: AbortSignal): Promise<EmbeddingResponse> {
    ...
    if (request.inputs.some((input) => typeof input !== 'string') || request.inputs.length === 0) {
      throw new ModelGatewayError(
        'Embedding inputs must contain at least one string',
        'VALIDATION',
        {},
        400
      );
