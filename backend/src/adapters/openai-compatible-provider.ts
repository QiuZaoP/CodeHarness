import type { ModelRouteConfig, GatewayConfig } from '../model-gateway/config.ts';
import { resolveApiKey } from '../model-gateway/config.ts';
...
import {
  ...
  withRetries,
  type Sleep
} from '../model-gateway/http.ts';
import type { ChatMessage } from '../model-gateway/prompts.ts';
...
function contentFromCompletion(payload: Record<string, unknown>): { content: string; model: string; usage: ProviderUsage } {
  const choices = payload.choices;
  if (!Array.isArray(choices) || choices.length === 0) {
    throw new ModelGatewayError('Model response has no choices', 'INVALID_RESPONSE', {}, 502);
  }
  const first = object(choices[0]);
  const message = object(first?.message);
  const content = message?.content;
  if (typeof content !== 'string') {
    throw new ModelGatewayError('Model response has no text content', 'INVALID_RESPONSE', {}, 502);
  }
  return {
    content,
    model: typeof payload.model === 'string' && payload.model ? payload.model : 'unknown-model',
...
export class OpenAICompatibleProvider {
  ...
  async chat(messages: ChatMessage[], options: ChatOptions): Promise<ChatResult> {
    ...
    const result = await withRetries(
      ...
      },
      {
        ...
        onRetry: () => {
          if (emitted) {
            throw new ModelGatewayError('Model stream failed after output started', 'PROVIDER', {}, 502);
          }
        }
      }
    );
    ...
    const result = await withRetries(
      async () => {
        ...
        if (!Array.isArray(data) || data.length !== inputs.length) {
          throw new ModelGatewayError('Embedding response shape is invalid', 'INVALID_RESPONSE', {}, 502);
        }
        const vectors = data
          .map((item) => {
        ...
        return {
          vectors,
          model: typeof payload.model === 'string' && payload.model ? payload.model : this.route.model,
    ...
    const result = await withRetries(
      async () => {
        ...
        const results = payload.results.map((item) => {
          ...
          if (typeof value?.index !== 'number' || typeof value.relevance_score !== 'number' || !Number.isFinite(value.relevance_score)) {
            throw new ModelGatewayError('Rerank response shape is invalid', 'INVALID_RESPONSE', {}, 502);
          }
  ...
  private async consumeStream(
    response: { body?: { getReader(): { read(): Promise<{ done: boolean; value?: Uint8Array }> } } | null },
    options: ChatOptions,
    markEmitted: () => void
  ): Promise<Omit<ChatResult, 'retries'>> {
    if (!response.body) throw new ModelGatewayError('Model stream has no response body', 'INVALID_RESPONSE', {}, 502);
    ...
    const consumeLine = (line: string) => {
      ...
      try {
        ...
        throw new ModelGatewayError('Model stream contained invalid JSON', 'INVALID_RESPONSE', {}, 502);
      }
      ...
      if (typeof delta === 'string' && delta.length > 0) {
        ...
        if (Buffer.byteLength(nextContent, 'utf8') > maxResponseBytes) {
          throw new ModelGatewayError('Model content exceeded the configured size limit', 'INVALID_RESPONSE', {
            maxResponseBytes
          }, 502);
        }
        content = nextContent;
    ...
    while (true) {
      ...
      if (responseBytes > maxResponseBytes) {
        throw new ModelGatewayError('Model stream exceeded the configured size limit', 'INVALID_RESPONSE', {
          maxResponseBytes
        }, 502);
      }
    ...
    if (!content) throw new ModelGatewayError('Model stream returned empty content', 'INVALID_RESPONSE', {}, 502);
